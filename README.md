...
are: $$\boxed{\,\psi_v(Q) \,=\, N_v\,y^{\lambda - v - 1/2}\,e^{-y/2}\,L_v^{(2\lambda - 2v - 1)}(y)\,}$$ where - $L_v^{(\alpha)}(y)$ is the associated Laguerre polynomial, and - the normalization constant is $$N_v \,=\, \sqrt{ \dfrac{a\,(2\lambda - 2v - 1)\,\Gamma(v+1)}{\Gamma(2\lambda - v)} } .$$
...
$$S_1 = -\frac{N_0 N_n}{a^2} \sum_{m=0}^{n} (-1)^m c_m I_m^{(1)}/m!.$$ - High-precision evaluation or Stirling/log-Gamma approximations are
recommended.
...
>>>>>>> 2b379bcf7386ce1c50a709d77d00b819ae239992
